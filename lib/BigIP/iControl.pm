...
=head3 grace_disable_pool_member ( $pool, $pool_member )

Gracefully disable the pool member in a pool.

=cut
sub grace_disable_pool_member {
    my ($self, $pool, $pool_member) = @_;
    $self->_grace_toggle_pool_member( $pool, $pool_member, "STATE_DISABLED" );
}


=head3 _grace_toggle_pool_member ( $pool, $pool_member, $action )

Gracefully toggle the pool member in a pool.

=cut

sub _grace_toggle_pool_member {
